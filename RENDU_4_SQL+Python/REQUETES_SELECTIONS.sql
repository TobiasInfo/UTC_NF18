...
------------------------------------REQUETE SQL------------------------------------
--REQUETE 1
SELECT nom_molecule, SUM(quantite)
    ...
    AND date_fin <= '2022-03-01' -- valeur a entrer via interface python par l'utilisateur
    GROUP BY nom_molecule;  


-- REQUETE 2 
SELECT COUNT(*) AS Nombre_de_traitements
    FROM Traitement 
    WHERE date_debut >= '2019-01-01' -- valeur a entrer via interface python par l'utilisateur
    AND date_debut <= '2022-01-01'; -- valeur a entrer via interface python par l'utilisateur
...
-- REQUETE 5
SELECT C.nom AS "nom du client", C.prenom AS "prenom du client",A.id_animal, A.nom "nom de l'animal", A.date_de_naissance AS "date de naissance de l'animal", num_puce, passeport, A.type
...
SELECT P.*, poste
    ...
    INNER JOIN NomPoste NP
    ON P.id_personnel = NP.id_personnel
    WHERE type = 'reptiles';
